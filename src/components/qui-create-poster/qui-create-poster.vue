<template>
	<view class="content" v-if="isShow" @click.stop="close">
		<uni-notice-bar v-if="showtip"  style="width:350rpx;height:100rpx;text-align: center;z-index:100000"  background-color="#000"  color="#fff" single="true" text="长按图片即可保存！"></uni-notice-bar>
		...
		<scroll-view scroll-y="true" style="max-height:90vh;width:100vw;position:fixed;left: 0;overflow:auto;" :class="canvasH>500?'alitem':'creatIMg'" v-if="tempImgShow">
			<image  class="showImg" @longpress="longpress" :style="{ width: canvasW + 'px', height: canvasH + 'px' }" :src="tempImg"></image>
		</scroll-view>
...
// }
	export default{
		props:{
			headerImg:{
				type: String,
				// default: 'https://pages.70e.com/storage/attachments/2020/08/08/L2TvT8xarUAOOTG475C4b36fv0tUE44p84xE1V9W_thumb.jpeg'
			},
			subTitle:{
				type: String,
	     ...
	     move(){
		  ...
		  //截取字符串，按照br换号，截取
		   spliceStr(str){
			...
			str=str.replace(spanReg, '').replace(/\s+/g, "");   //去除空格已经话题@等内容
			console.log(str)
             console.log(str);
			if(str.indexOf('<br>') !=-1){
